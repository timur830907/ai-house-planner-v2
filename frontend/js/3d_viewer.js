...
function update3DLayout(layoutData) {
  ...
  const rooms = layoutData.rooms || layoutData.layout || layoutData;
  ...
  Object.keys(rooms).forEach((roomName) => {
    ...
    // Стены
    const wBoxGeo = new THREE.BoxGeometry(width, wallHeight, wallThickness);
    const dBoxGeo = new THREE.BoxGeometry(wallThickness, wallHeight, depth);

    const wallS = new THREE.Mesh(wBoxGeo, wallMat);
    wallS.position.set(x1 + width / 2, wallHeight / 2, y1);
    layoutGroup.add(wallS);

    const wallN = new THREE.Mesh(wBoxGeo, wallMat);
    wallN.position.set(x1 + width / 2, wallHeight / 2, y2);
    layoutGroup.add(wallN);

    const wallW = new THREE.Mesh(dBoxGeo, wallMat);
    wallW.position.set(x1, wallHeight / 2, y1 + depth / 2);
    layoutGroup.add(wallW);

    const wallE = new THREE.Mesh(dBoxGeo, wallMat);
